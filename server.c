...
//		error_handling ("fopen() error"); 
	...
	rcvdFileSize = atoi(rcvFileName);
	void* a;
//	while (rcvdFileSize < fileSize)
//	{ 
	//	numBytesRcvd = recv (clnt_sock, frame[frame_num]+rcvdFileSize, FRAME_SIZ-rcvdFileSize, 0); 
		numBytesRcvd = recv (clnt_sock, frame[frame_num], rcvdFileSize, MSG_WAITALL); 
		if (numBytesRcvd == -1) {
			printf("oops\n");
			error_handling ("recv() error"); 
		}
